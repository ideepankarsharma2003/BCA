...
void boundary_fill(int pos_x, int pos_y, int fill_color, int boundary_color)
{
    ...
    if (current_color != boundary_color && current_color != fill_color) // if pixel not already filled or part of the boundary then
    {
        ...
        boundary_fill(pos_x + 1, pos_y+1, boundary_color, fill_color); // perform same function for the south-east pixel
        boundary_fill(pos_x - 1, pos_y-1, boundary_color, fill_color); // perform same function for the north-west pixel
        boundary_fill(pos_x-1, pos_y + 1, boundary_color, fill_color); // perform same function for the north-east pixel
        boundary_fill(pos_x+1, pos_y - 1, boundary_color, fill_color); // perform same function for the south-west pixel
    }
}
